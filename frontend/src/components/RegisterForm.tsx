...
export function RegisterForm({ 
  ...
  const [formData, setFormData] = useState({
    ...
    firstName: '',
    lastName: '',
    // No role field - students automatically get BUYER+SELLER from backend
  });
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Register as student - backend assigns BUYER+SELLER roles automatically
      const payload = {
        ...
        // No role field - let backend assign default roles
      };
      ...
      if (onRegister) {
        // Pass roles array from result
        onRegister(result?.roles || ['BUYER', 'SELLER']);
      }
    } catch (err: any) {
